...
void decode(int* received_vector, int* decoded_vector, int block_size, int num_info_bits){
	...
	*/
	...
	for (int i = 0; i < block_size; i++) {
		syndrome_register_inputs[0] = syndrome_register_outputs[7] ^ received_vector[block_size - 1 - i];
		syndrome_register_inputs[1] = syndrome_register_outputs[0];
		syndrome_register_inputs[2] = syndrome_register_outputs[1];
		syndrome_register_inputs[3] = syndrome_register_outputs[2];
		syndrome_register_inputs[4] = syndrome_register_outputs[3] ^ syndrome_register_outputs[7];
		syndrome_register_inputs[5] = syndrome_register_outputs[4];
		syndrome_register_inputs[6] = syndrome_register_outputs[5] ^ syndrome_register_outputs[7];
		syndrome_register_inputs[7] = syndrome_register_outputs[6] ^ syndrome_register_outputs[7];
		for (int j = 0; j < num_parity_bits; j++) {
			syndrome_register_outputs[j] = syndrome_register_inputs[j];
		}
	}
	int A1 = syndrome_register_outputs[3];
	int A2 = syndrome_register_outputs[1] ^ syndrome_register_outputs[5];
	int A3 = syndrome_register_outputs[0] ^ syndrome_register_outputs[2] ^ syndrome_register_outputs[6];
	int A4 = syndrome_register_outputs[7];
	int M = A1&A2&A3 | A2&A3&A4 | A1&A3&A4 | A1&A2&A4;
	decoded_vector[block_size - 1] ^= M;
	syndrome_register_inputs[0] = syndrome_register_outputs[7] ^ M;
	int temp = decoded_vector[block_size - 1];
	...
	for (int i = 1; i < block_size; i++) {
		...
		int A3 = syndrome_register_outputs[0] ^ syndrome_register_outputs[2] ^ syndrome_register_outputs[6];
		int A4 = syndrome_register_outputs[7];
		int M = A1&A2&A3 | A2&A3&A4 | A1&A2&A4 | A1&A3&A4;
		decoded_vector[block_size - 1] ^= M;
		syndrome_register_inputs[0] = syndrome_register_outputs[7] ^ M;
		int temp = decoded_vector[block_size - 1];
		for (int j = block_size - 1; j > 0; j--) {
